...
def is_valid_password(password):
    ...
    for sth in password: # start to calculate the number
        if sth.isdigit():
            ...
            count_special += 1
    if count_digit == 0 or count_upper == 0 or count_lower == 0:  # Set the situation with none numbers include
        return False

    if SPECIAL_CHARS_REQUIRED:   # set the situation of required special charactor
        if count_special == 0:
            return False
