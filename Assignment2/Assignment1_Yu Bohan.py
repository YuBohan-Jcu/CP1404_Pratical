...
Q - Quit'''   # To set the menu list
    ...
    def main(self):
        while True:   # To set a recycle make sure it can be asked when the wrong choose comes out
            ...
            if choose == "L":  # To explain l-choice
                self.get_l(songs_list, total, count, counts)
            elif choose == "A":  # To explain a_choice
                self.get_a(songs_list)
            elif choose == "C":  # TO explain c_choice
                self.get_c(songs_list)
            elif choose == "Q":  # When you quit, what will happen
                self.get_q(songs_list)
                print(f"{counts} songs has been loaded in data.csv")  # Count how many song we already learned
                print("Have a nice day!")
                break   # quit

    def get_l(self, songs_list, total, count, counts):
    ...
    def get_a(self, songs_list):   # to explain when the function of a_choice
    ...
    def get_q(self, songs_list):   # when u quit what will happen
